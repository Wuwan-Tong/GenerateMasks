...
% set at line 9 if you need original masks or complement
...
% if you want to include more block size at line 28 please change the code from line 67 to line 99 accordingly
% set which blocksize requires values in heatmap at the if code on line 76
%% set parameters
...
for iw=1:length(block_size_w) % block size
    for ih=1:length(block_size_h)
        ...
        for wtype=1:16 % wedge type
            for ipos_w=1:w % position in each DCT bases
                for ipos_h=1:h
