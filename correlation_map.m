...
% set at line 7 if you need original masks or complement
% set the path to load DCT dictionaries, set the path to save correlation data and .png figures
% set which blocksize requires values in heatmap at the if code on line 81
% if you only need data, without png figures, comment out the code between 'plot start-----' and 'plot end-----'
% if you want to include more block size on line 30, please change the code from line 69 to line 99 accordingly
%% set parameters
...
for iw=3%1:length(block_size_w) % block size
    for ih=3%1:length(block_size_h)
        ...
        for wtype=1:16 % wedge type
            for ipos_w=1:w % position on DCT Dictionary (position on DCT bases)
                for ipos_h=1:h
                    ...
                    if w==8&&h==8 % show values in the cell, set which blocksize requires values in heatmap
                    ...
                    % save file
                    filename_fig=[path_corr_png,'/w', num2str(w),'_h', num2str(h), '/y',num2str(ipos_h), '_x',num2str(ipos_w),'_wtype',num2str(wtype)];
